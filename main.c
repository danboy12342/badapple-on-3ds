...
int main(int argc, char **argv)
{
	...
	//^ un comment !NOT FOR PUBLIC!

	printf("Wolfvak and Daniel ShengYi Presents:\n");
	printf("Bad Apple!!! PV on 3ds!\n");
	printf("Song: Bad Apple!!! feat nomico\n");

	res = romfsInit();
	if (R_FAILED(res)) {
		printf("Error at romfsInit()!???\n"); //some awful err messages, too bad!
		goto end;
	}
	...
	while (aptMainLoop()) {
		...
		if (hidKeysDown() & KEY_START)
			break; // break in order to return to hbmenu (if not running on hbmenu oh well sucks to be you)
	}
