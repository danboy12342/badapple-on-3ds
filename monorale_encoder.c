...
#include <assert.h>
#include <stdint.h>
//this is fine vim is lying to you
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define SCREEN_SZ		(400*240) /*update for XL?*/ //no its fine /*sure sure*/

typedef uint16_t rle_t;

#define RLE_MAX (1ULL << (sizeof(rle_t) * 8)) //this works, could be better
	...
	for (size_t i = 0; i < framecnt; i++) {
		...
		while(1) {
			...
			while((vidbuf[vidpx] < GREY_THRESHOLD) && (vidpx < SCREEN_SZ)) {
				 ...
				 * here.
				 */
					break; //for cia only replace with ex
				}
			}
